...
This is the newest version of this file! This module has been put in the
'AtmosGasTools' package.

This module contains class definitions and functions which are designed to:
contain, process, and display time series data of atmospheric gases.

NB:
#Instances of AtmosGasTS and it's child classes are refered to as gas objects
#File paths must be relative to the calling script (not this, the defining
 script)

Available classes:
--AtmosGasTS: contains methods and attributes for a generic gas time series
--WDCGG_TS: child class of AtmosGasTS, automates initilasation using data from
            the World Data Centre for Greenhouse Gases (link in class
            docstring)
--JOIN_TS: child class of AtmosGasTS, automates initilasation using data from
           the Jülich Open Web Interface (link in class docstring)
--GasCorrelation: compares two input gas classes and finds relationships

Available functions:
-plot_multi_ts: on the same axes, plots time series of multiple gas objects
-plot_multi_ps: on the same axes, plots power spectra of multiple gas objects
-freq_bounds: computes the powers (base 10) of the sampling frequencies
-round_to_n: rounds an input to a given number of significant figures
...
class AtmosGasTS(object):
    ...
    def find_interpol(self, raw=False, order='linear'):
        ...
        interp1d.html#scipy.interpolate.interp1d
    ...
    def plot_ts(self, join_data=False, save=False, raw=False, plot_line=False):
        """
        ...
        if plot_line:
            inc = round_to_n(self.line_stats[0]*365*24*60*60, 3)
            inc_lab = 'Surface change: ' + str(inc)
            inc_lab += ' ' + self.g_unit + ' per year'
            plt.plot(times, self.line, label=inc_lab)
        else:
            pass
        ...
        plt.legend(loc='best')
        if save:
            plt.savefig(self.path + '/' + self.t_name + '.pdf', format='pdf')
            plt.close()
    ...
    def plot_ps(self, join_data=False, save=False, days=False, rel=False):
        """
        ...
        if save:
            plt.savefig(self.path + '/' + f_name_new + '.pdf', format='pdf')
            plt.close()
            print('Figure saved as:')
    ...
    def __data(self):
        ...
        for row in self.file_data:
            ...
            if gas >= 0:
                ...
                #print('Gas concentration is negative; row', index,
                #     'not included:')
                #print(row)
                pass
            else:
        ...
        with open(path, 'r') as tf:
            for index, row in enumerate(tf):
                ...
                if row[0] == '#':
                    ...
                    yr, mth, dy = int(yr), int(mth), int(dy)
                    hr, mins = int(hr), int(mins)

                    if float(conc) >= 0:
                        datetime_obj = datetime.datetime(yr, mth, dy, hr, mins)
                        datetime_arr = np.append(datetime_arr, datetime_obj)
                        gas_concs = np.append(gas_concs, float(conc))
                    else:
                        pass
        return header, datetime_arr, gas_concs
...
class GasCorrelation(object):
    ...
    def plot_ratio_spectrum(self, title='', fig_name='', join_data=False,
                            save=False, days=False, rel=False):
        """
        Plots a spectrum of those produced by 'periodogram'.
        ...
        if save:
            plt.savefig(self.path + '/' + fig_name + '.pdf', format='pdf')
            plt.close()
            print('Figure saved as:')
    ...
    def plot_ratio_ts(self, title='', fig_name='', join_data=False,
        ...
        if self.g0_unit == self.g1_unit:
            ...
            units = self.g0_unit + '/' + self.g1_unit
            ylab += ' / ' + units
        ...
        if trend:
            inc = round_to_n(self.trend_stats[0]*365*24*60*60, 3)
            inc_lab = 'Surface change: ' + str(inc)
            inc_lab += ' ' + units + ' per year'
            plt.plot(self.t, self.trend, label=inc_lab)
        else:
            pass
    ...
    def plot_scatter(self, title='', fig_name='', save=False, line=True):
        """
...
def alph(i, factor=0.5):
    """
    returns an alpha (opacity) value based on an input index
    """
    return np.exp(-factor*i)


def plot_multi_ts(gas_obj_arr, title='', fig_name='', join_data=False,
                  save=False, raw=False, lab_arr=None):
    """
    ...
    lab_arr -- array; array of strings which will become the labels for each
               gas object
    """
    ...
    for index, gas_obj in enumerate(gas_obj_arr):
        ...
        if join_data:
            plt.plot(times, gas_concs, label=lab, alpha=alph(index))
        else:
            plt.plot(times, gas_concs, ls='', marker='+', ms=1, label=lab,
                     alpha=alph(index))
...
def plot_multi_ps(gas_obj_arr, title='', fig_name='', join_data=False,
                  save=False, days=False, lab_arr=None):
    """
    ...
    for index, gas_obj in enumerate(gas_obj_arr):
        ...
        if join_data:
            plt.plot(indep_var, dep_var, label=lab, alpha=alph(index))
        else:
            plt.plot(indep_var, dep_var, ls='', marker='+', ms=1, label=lab,
                     alpha=alph(index))
...
#add this to AtmosGasTS at some point
def freq_bounds(samp_period, series_length, lfactor=1):
    """
    Computes the powers (base 10) of the bounds of the sampling frequencies;
    intended for use with the 'fast_find_spectrum' method of the gas objects

    samp_period -- float; time between samples in hours

    series_length -- float; length of the time series in years

    lfactor -- float; uppber bound period = series_length*lfactor
    """
    lb_period = 2*60*60*samp_period
    mag_high = np.log10(1/lb_period)

    ub_period = 60*60*24*365.25*series_length*lfactor
    mag_low = np.log10(1/ub_period)
    return mag_low, mag_high
...
def round_to_n(x, n):
    ...
    n -- int; number of significant figures to use

    Thanks to 'Greg'
    http://stackoverflow.com/questions/18915378/rounding-to-significant-figures
    -in-numpy
    """
